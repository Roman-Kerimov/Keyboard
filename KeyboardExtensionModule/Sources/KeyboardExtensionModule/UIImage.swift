...
extension UIImage {
    public convenience init?(fromPDF path: String, withExtension pdfExtension: PDFExtension, withScale scale: CGFloat) {
        
        guard let pdfURL = Bundle.module.url(forResource: path, withExtension: pdfExtension.rawValue) else {
            return nil
        }
